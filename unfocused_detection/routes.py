...
unfocused = Blueprint('unfocused', __name__)
...
def unfocused_detection():
    detected_objects = DetectedObject.query.join(Detector).filter(Detector.detector_type == 'Unfocused').order_by(DetectedObject.timestamp.desc()).all()
...
def view_object(object_id):
    ...
    return render_template('unfocused/view_object.html', detected_object=detected_object)
