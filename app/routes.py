...
def dashboard():
    ...
    num_no_helmet = DetectedObject.query.filter(DetectedObject.name == "no-helmet").count()
    num_no_vest = DetectedObject.query.filter(DetectedObject.name == "no-vest").count()
    num_no_ppe = num_no_helmet + num_no_vest
    num_drowsy = DetectedObject.query.filter(DetectedObject.name == "drowsy").count()
    num_phone = DetectedObject.query.filter(DetectedObject.name == "phone").count()
    num_no_seatbelt = DetectedObject.query.filter(DetectedObject.name == "no-seatbelt").count()
    num_reckless = num_drowsy + num_phone + num_no_seatbelt
    num_danger = DetectedObject.query.filter(DetectedObject.name == "cross-hand").count()
    ...
    if selected_camera_ppe == 'all':
        ...
        total_no_helmet_all = db.session.query(DetectedObject).filter(DetectedObject.name == 'no-helmet').count()
        total_no_vest_all = db.session.query(DetectedObject).filter(DetectedObject.name == 'no-vest').count()
    else:
        # Mengambil data berdasarkan kamera yang dipilih
        data = db.session.query(DetectedObject).join(Detector).join(CCTV) \
            .filter(CCTV.id == selected_camera_ppe).all()
    ...
    if selected_camera_driver == 'all':
        # Mengambil semua data untuk semua kamera
        total_drowsy_all = db.session.query(DetectedObject).filter(DetectedObject.name == 'drowsy').count()
        total_no_seatbelt_all = db.session.query(DetectedObject).filter(DetectedObject.name == 'no-seatbelt').count()
        total_phone_all = db.session.query(DetectedObject).filter(DetectedObject.name == 'phone').count()
        total_reckless_all = total_drowsy_all + total_no_seatbelt_all + total_phone_all
    else:
        # Mengambil data berdasarkan kamera yang dipilih
        data = db.session.query(DetectedObject).join(Detector).join(CCTV) \
            .filter(CCTV.id == selected_camera_driver).all()

        total_drowsy_all = db.session.query(DetectedObject).join(Detector).join(CCTV) \
            .filter(CCTV.id == selected_camera_driver, DetectedObject.name == 'drowsy').count()
        total_no_seatbelt_all = db.session.query(DetectedObject).join(Detector).join(CCTV) \
            .filter(CCTV.id == selected_camera_driver, DetectedObject.name == 'no-seatbelt').count()
        total_phone_all = db.session.query(DetectedObject).join(Detector).join(CCTV) \
            .filter(CCTV.id == selected_camera_driver, DetectedObject.name == 'phone').count()
        total_reckless_all = total_drowsy_all + total_no_seatbelt_all + total_phone_all
    ...
    if selected_camera_danger == 'all':
        # Mengambil semua data untuk semua kamera
        total_gesture_help_all = db.session.query(DetectedObject).filter(DetectedObject.name == 'cross-hand').count()
    else:
        # Mengambil data berdasarkan kamera yang dipilih
        data = db.session.query(DetectedObject).join(Detector).join(CCTV) \
            .filter(CCTV.id == selected_camera_danger).all()
        total_gesture_help_all = db.session.query(DetectedObject).join(Detector).join(CCTV) \
            .filter(CCTV.id == selected_camera_danger, DetectedObject.name == 'cross-hand').count()
    ...
    if selected_camera_ppe == 'all':
        num_no_vest_per_day = [
            db.session.query(DetectedObject).filter(
        ...
        num_no_vest_per_day = [
            db.session.query(DetectedObject).join(Detector).join(CCTV).filter(
                DetectedObject.name == 'no-vest',
    ...
    if form.validate_on_submit():
        if detector:
            ...
            new_detector = Detector(
                ...
                detector_type_id=form.types.data,
