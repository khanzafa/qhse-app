...
def dashboard():
    ...
    cctv_locations = (db.session.query(subquery.c.name).order_by(subquery.c.id)).all()
    ...
    ###########################
    all_cctv = (
        db.session.query(CCTV).filter_by(permission_id=session_permission).count()
    )
    all_detectors = (
        db.session.query(Detector).filter_by(permission_id=session_permission).count()
    )
    all_weight = (
        db.session.query(Detector)
        .filter(
            Detector.permission_id == session_permission, Detector.weight_id.isnot(None)
        )
        .count()
    )
    all_detection = (
        db.session.query(DetectedObject)
        .filter_by(permission_id=session_permission)
        .count()
    )
    ...
    daily_detected_results = (
        detected_objects_query.filter(DetectedObject.timestamp >= start_time)
        .with_entities(
            db.func.date_trunc("hour", DetectedObject.timestamp).label(
                "detection_hour"
            ),
        ...
        .group_by(
            db.func.date_trunc("hour", DetectedObject.timestamp),
            CCTV.type,
            DetectedObject.name,
        )
    ...
    for row in daily_detected_results:
        ...
        daily_counts_dict[detection_hour] = (
            daily_counts_dict.get(detection_hour, 0) + count
        )

        daily_object_types_dict[object_name] = (
            daily_object_types_dict.get(object_name, 0) + count
        )

        daily_pie_chart_types_dict[cctv_type] = (
            daily_pie_chart_types_dict.get(cctv_type, 0) + count
        )

    daily_labels = [
        (current_date - timedelta(hours=i)).replace(minute=0, second=0, microsecond=0)
        for i in range(24)
    ]
    ...
    daily_object_counts = {
        object_name: 0 for object_name in daily_object_types_dict.keys()
    }
    ...
    daily_pie_chart_counts_dict = {
        cctv_type: 0 for cctv_type in daily_pie_chart_types_dict.keys()
    }
    ...
    weekly_detected_results = (
        detected_objects_query.filter(DetectedObject.timestamp >= seven_days_ago)
        .with_entities(
        ...
        .group_by(
            db.func.date_trunc("day", DetectedObject.timestamp),
            CCTV.type,
            DetectedObject.name,
        )
    ...
    for row in weekly_detected_results:
        detection_day = row.detection_day.strftime(
            "%Y-%m-%d"
        ...
        weekly_counts_dict[detection_day] = (
            weekly_counts_dict.get(detection_day, 0) + count
        )

        weekly_object_types_dict[object_name] = (
            weekly_object_types_dict.get(object_name, 0) + count
        )

        weekly_pie_chart_types_dict[cctv_type] = (
            weekly_pie_chart_types_dict.get(cctv_type, 0) + count
        )

    weekly_labels = [
        (current_date - timedelta(days=i)).strftime("%Y-%m-%d") for i in range(7)
    ]
    ...
    weekly_object_counts_dict = {
        object_name: 0 for object_name in weekly_object_types_dict.keys()
    }
    ...
    weekly_pie_chart_counts_dict = {
        cctv_type: 0 for cctv_type in weekly_pie_chart_types_dict.keys()
    }
...
def detected_object():
    ...
    if search_query:
        detected_objects = (
            ...
            .join(CCTVLocation)
            .join(Weight)
            .join(DetectorType, Weight.detector_type_id == DetectorType.id)
            .filter(
                db.or_(
...
@report_bp.route("/detected-object/download_detected_objects_excel", methods=["GET"])
@login_required
def download_detected_objects_excel():
    logger.info("Download detected objects Excel initiated.")
    ...
    try:
        ...
        query = (
            db.session.query(
                DetectedObject.id,
                DetectedObject.name,
                CCTVLocation.name.label("cctv_location_name"),
                CCTV.type.label("cctv_type"),
                DetectorType.name.label("detector_type_name"),
                DetectedObject.timestamp,
            )
            .select_from(DetectedObject)  # Specify the base entity
            .join(Detector)
            .join(CCTV)
            .join(CCTVLocation)
            .join(Weight)
        ...
        # Load query results into a list of dictionaries
        detected_objects = query.all()
        detected_objects_json = [
            {
                "id": obj.id,
                "name": obj.name,
                "cctv location": obj.cctv_location_name,
                "cctv type": obj.cctv_type,
                "detector type": obj.detector_type_name,
                "date": obj.timestamp.strftime("%Y-%m-%d"),  # Extract date
                "time": obj.timestamp.strftime("%H:%M:%S"),  # Extract time
            }
        ...
        with open("detected_objects.json", "w") as f:
            json.dump(detected_objects_json, f)
        logger.info("JSON file generated successfully.")

        # Load JSON data into a DataFrame
        df = pd.read_json("detected_objects.json")
        logger.info("Data loaded into DataFrame successfully.")

        # Write DataFrame to Excel
        output = BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            df.to_excel(writer, index=False, sheet_name="Detected Objects")
        output.seek(0)
